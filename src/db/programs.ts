...
const programs: Program[] = [
  ...
  },
  {
    ...
    order: 2,
    title:
      '비동기 코드 없이 게임 리소스 데이터 사용하기 (feat. React Suspense)',
    description:
      '설치 시 필요한 모든 리소스 데이터를 다운받아 놓을 수 있는 게임 클라이언트와 다르게, 웹 어플리케이션은 이런 구조를 가지기 어렵습니다. 기존 쿠키런: 킹덤 어드민 툴의 데이터 로딩 방식은 증가하는 게임 데이터를 감당하기 어렵거나, 로딩 관련 코드가 많아지는 문제점 둘 중 하나를 피하기 어려웠습니다. 이를 개선하기 위해 React Suspense를 도입하며 두 가지 문제를 모두 해결하는 구조를 고안하고, 그 과정에서 마주친 문제를 해결한 방법을 공유해 보고자 합니다.',
    speaker: {
      name: '문태근',
      company: '데브시스터즈',
    },
